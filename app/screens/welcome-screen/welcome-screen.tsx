...
export const WelcomeScreen = observer(function WelcomeScreen() {
	...
	const callApi = () => {
		...
		api
			.login(uuid.v1(), username, password)
			.then(async (loginData) => {
				Alert.alert(JSON.stringify(loginData))
				console.log("finishedRegister: " + String(loginData.finishedRegister))
				console.log("isNewDevice: " + String(loginData.isNewDevice))
				console.log("newDeviceToken: " + loginData.newDeviceToken)
				// if (loginData.newDeviceToken.length > 0) {
				// 	try {
				// 		await AsyncStorage.setItem(constant.asyncStorageKey.newDeviceToken, loginData.newDeviceToken)
				// 	} catch (error) {
	...
	const showNewDeviceToken = async () => {
		const newDeviceToken = await AsyncStorage.getItem(constant.asyncStorageKey.newDeviceToken)
		Alert.alert(newDeviceToken)
	}

	return (
		<View style={FULL}>
			<View style={LOADING}>{isLoading ? <ActivityIndicator /> : null}</View>
			<Screen style={CONTAINER} preset="scroll" backgroundColor={color.transparent}>
				...
				<View style={FORGET_CONTAINER}>
					<Button style={FORGET_LEFT}>
						{/*  onPress={nextScreen} */}
						<Text tx="loginScreen.forgetName" style={FORGET_TEXT} />
					</Button>
					<Button style={FORGET_RIGHT}>
						{/* onPress={nextScreen} */}
